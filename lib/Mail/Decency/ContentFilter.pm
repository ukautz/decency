...
=head2 session : Mail::Decency::Core::SessionItem::ContentFilter

SessionItem (L<Mail::Decency::Core::SessionItem::ContentFilter>) of the current handle file

=cut

has session => ( is => 'rw', isa => 'Mail::Decency::Core::SessionItem::ContentFilter' );
...
sub session_init {
    ...
    # setup new info
    ( my $init_id = $file ) =~ s/[\/\\]/-/g;
    $self->session( Mail::Decency::Core::SessionItem::ContentFilter->new(
...
sub _save_mail_to_dir {
    ...
    copy( $self->current_file, $failure_file );
}
