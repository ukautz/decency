...
=head2 SIMPLE EXAMPLE
    ...
    sub handle {
        ...
        # set a flag for later evaluation (eg in ContentFilter)
        $self->set_flag( 'bla' );
...
To include the module, simple add it in your policy configuration
...
You can use the (get|set|del|has)_flag-methods to communicate informations. The flags will also be transported in the decency X-Decency-Details header to the content filter (if forward_scoring is enabled)
