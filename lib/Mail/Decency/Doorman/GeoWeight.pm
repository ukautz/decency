package Mail::Decency::Doorman::GeoWeight;

use Mouse;
use mro 'c3';
extends qw/
    Mail::Decency::Doorman::Core
    Mail::Decency::Doorman::Model::GeoWeight
/;
...
Mail::Decency::Doorman::GeoWeight
...
sub handle {
    ...
    my $country = $self->geo_ip->country_code_by_addr( $self->ip );
    
    # no country determiend .. probably LAN ip ..
    return unless $country;
    
    # remember country in session
    $self->session->set_val( country => $country );
    ...
    if ( $self->enable_weight ) {
        ...
        ;
        if ( $weight ) {
            $self->add_spam_score( $weight, message_and_detail => "GeoWeight: $weight" );
        }
    }
