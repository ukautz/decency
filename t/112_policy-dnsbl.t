...
CHECK_NEGATIVE: {
    ...
    ok( 
        $server->session->spam_details_str( ' / ' ) eq 'Module: Test; Score: 0; No hit on DNSBLs'
...
CHECK_POSITIVE: {
    ...
    ok( 
        $server->session->spam_details_str( ' / ' ) eq 'Module: Test; Score: -100; ix.dnsbl.manitu.net: hit (-100); Blacklisted on: ix.dnsbl.manitu.net'
        && $server->session->message_str( ' / ' ) eq 'ix.dnsbl.manitu.net: hit (-100); Blacklisted on: ix.dnsbl.manitu.net',
